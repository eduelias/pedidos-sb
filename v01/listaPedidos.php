...
<body>
<h3>Lista de pedidos em aberto</h3>
...
<?
	...
	$sql = "select distinct p.id, p.dataPedido, c.codFilial, c.razaoSocial  
	          from cabPedido p, cliente c where p.idCliente = c.id and p.idCoordenador = $idCoordenador order by $order"; // group by p.id, p.dataPedido, c.codFilial, c.razaoSocial";
	$res = mysql_query($sql);
