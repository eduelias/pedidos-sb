...
<?
	...
	if ($acao == 'Gravar') {
		...
		echo '<table border="0">';
		echo '<tr>';
		echo '<td>Quant</td><td>Und</td><td>Tp</td><td>Descricao</td>'; 
		echo '</tr>';
		...
		$sql = "select *,p.id as idProduto, p.descricao as descProduto from produto p, tabelaPreco t
		...
		while ( $res = mysql_fetch_array($ret)) {
			...
			if ( $$qtProduto != '' ) {
				...
				$sql = "insert into itemPedido (idCabPedido, idProduto, quantidade, preco, tipo)
						values ( $idCabPedido,$idProduto, $quant, 0, '$tipo')";
...
<?    
	...
	$idRede = $ret['idRede'];
	$dataEntrega = $diaDataEntrega.'/'.$mesDataEntrega.'/'.$anoDataEntrega;
